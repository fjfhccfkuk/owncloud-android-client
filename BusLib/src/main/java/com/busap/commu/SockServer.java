    ...
    private boolean initServer() {
        ...
        try {
            ...
            if (!mSockSelector.isOpen()) {
                BusLog.write("CenterServer", " selector is closed.");
                return false;
            }
...
//            rootChnel.socket().bind(new InetSocketAddress(LOCAL_SER_IP, LOCAL_SER_PORT));
            ...
            BusLog.write("CenterServer", " 创建服务端socket失败,将被关闭。Excp:" + e.toString());
            try {
    ...
    private void handleSelecttionKey(SelectionKey k) {
        if (k.isAcceptable()) {
            try {
                ...
                BusLog.write("CenterServer", " handleSelecttionKey() excp:" + e.toString());
            } finally {
                return;
            }
        }
    ...
    private void handleSocketChannel() {
        if (null == mSockSelector || !mSockSelector.isOpen()) {
            BusLog.write("CenterServer"," handleSocketChannel() selector invalid.");
            return;
        }
    ...
    private class ServerRunnable implements Runnable {
        ...
        public void run() {
            if (!initServer()) {
                BusLog.write("CenterServer", " Failed to init CenterServer");
                return;
            }
